...
The Trace Control Interface spec, Table 4 cite:[riscv-trace-control-spec-1.0(Table 4)], has several reserved address ranges labeled "Reserved for future standard extension". The Event Trace fits into one of those and currently allocates and uses addresses *0x100-0x1A4*. Thus Event Trace control registers starting point is an offset from the Trace Encoder base address, i.e. *trTeEventBase = trBaseEncoder+0x100*.
...
which is defined in the RISC-V Trace Control Interface spec as *"Protocol defined trace mode"* cite:[riscv-trace-control-spec-1.0(Trace Encoder Control Interface)].

[#time-sync-message]
=== Time Synchronization Message for simultaneously recording mtime & timestamp
(((N-Trace, Time Synchronization Message)))
...
*Interval Count* The interval count is 2^(7+trTsSyncMax) where *trTsSyncMax* usable values ranges from 1 to 15, where 0 is *"off"*. This provides a range from 2^8 (256) to 2^22 (~4x10^6). Note that this field is WARL so some of the upper values could be left out to reduce hardware.
