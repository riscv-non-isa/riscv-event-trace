...
(((Filtering, Global and per-Event)))

These global and per-event filters are documented in the *RISC-V Trace Control Interface Specification* cite:[riscv-trace-control-spec-1.0(Ch. 6.3)]. These are documented here since they are already documented and the specification ratified.
...
(((Filtering, Interrupt Cause)))

NOTE: Several of the registers are "shared" with Ecause filtering, thus the name of the field may not match the comparison being made.  For example, trTeFilterMatchChoiceEcauseLow is used to compare against the cause value for both exceptions and interrupts.
